...
mdb <- mfdb(file.path(base_dir, 'db', 'atlantisiceland.duckdb'))
...
## biomass
tmp <- 
  iceom_ms_ind$survObsBiomB$IGFS_allbox_effic1[[1]] %>% 
  mutate(sampling_type = 'IGFS') %>%
  bind_rows(iceom_ms_ind$survObsBiomB$AUT_allbox_effic1[[1]] %>%
...
mfdb_import_survey(mdb, tmp, data_source = 'atlantis_survey_biomass')
...
## IGFS
tmp <-
  iceom_ms_comp$survObsLenComp$IGFS_allbox_effic1[[1]] %>% 
  mutate(sampling_type = 'IGFS') %>%
  bind_rows(
    iceom_ms_comp$survObsLenComp$AUT_allbox_effic1[[1]] %>% 
      mutate(sampling_type = 'AUT')) %>%
  bind_rows(
    iceom_ms_comp$fishObsLenComp[[1]] %>% 
