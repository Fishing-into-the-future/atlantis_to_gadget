...
sample_survey_biomass_box <- function(dat,cv,wtAtAge) {
  ...
  Bbox <- aggregate(list(biomassBox = dat2$biomass),
                    by = list(species = dat2$species, 
                              polygon = dat2$polygon,
                              time = dat2$time),
                    sum)
  ...
  Bbox2 <- merge(Bbox, totB, by = c('species', 'time'), all.x = TRUE)
  Bbox2$propB <- Bbox2$biomassBox/Bbox2$biomass
  
  # Bbox3 <- merge(Bbox2,totBobs,by="species",all.x=TRUE)
  # Bbox3$obsBbox <- Bbox3$obsBiomass*Bbox3$propB
  
  Bbox3 <- merge(Bbox2,totBobs,by=c("species", 'time'),all.x=TRUE)
  Bbox3$obsBbox <- Bbox3$obsBiomass*Bbox3$propB
