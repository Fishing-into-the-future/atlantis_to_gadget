...
create_sim_fishery_index_isl <- function(atlmod,fitstart=NULL,fitend=NULL,saveToData=T) {
  ...
  #atlantisom::read_savedfisheries(file.path(d.name, v.name), 'Catch')
  catchbio_ss <- truth$catch %>% 
    filter(species %in% species_ss) %>% 
    group_by(species, polygon, time) %>% 
    summarise(atoutput = sum(atoutput), .groups = 'drop')
  ...
  # for(f in names(catchbio_ss)[names(catchbio_ss) %in% fcvlook$fishery]){
    catchbio <- catchbio_ss %>%
      ...
      dplyr::left_join(dplyr::select(omlist_ss$funct.group_ss, species = Code, Name), by = c("Name")) %>%
      dplyr::mutate(ModSim = modsim) %>%
      dplyr::mutate(fishery = 'SEA') %>%
      #dplyr::mutate(area = 1) %>%
      dplyr::left_join(fcvlook %>% rename(Name = species)) %>%
      dplyr::select(ModSim, year, fishMonth, area, Code=species, Name, fishery, everything()) %>%
