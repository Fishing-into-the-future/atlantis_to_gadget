...
create_sim_biolpar <- function(atlmod,saveToData=T) {
  ...
  #  dplyr::left_join(omlist_ss$biol$kup, by=c("Code"="group")) %>%
    dplyr::left_join(omlist_ss$biol$time_spawn %>% rename('Code' = 1, 'DaySpawn' = 2), by=c("Code")) %>%
    dplyr::mutate(SpawnMonth = ceiling(DaySpawn/(365/12))) %>% 
    dplyr::left_join(omlist_ss$biol$recruit_time %>% rename('Code' = 1, 'recruit_time' = 2), by=c("Code")) %>%
    dplyr::mutate(RecruitMonth = ceiling((DaySpawn+recruit_time)/(365/12))) %>%
    dplyr::left_join(omlist_ss$biol$agespercohort %>% rename('Code' = 1, 'agespercohort' = 2), by=c("Code")) %>%
    dplyr::left_join(omlist_ss$biol$maturityogive %>% rename('Code' = 1, 'maturityogive' = 2), by=c("Code")) %>%
    dplyr::arrange(Name) %>%
    ...
    dplyr::select(ModSim, Code, Name, WLa = a, WLb = b, 
                  ...
                  NAgecl = NumCohorts, agecl1:agecl20) %>%
  ...
  maturitypar <- omlist_ss$funct.group_ss %>% 
    ...
    dplyr::arrange(Name) %>%
    dplyr::mutate(ModSim = modsim) %>%
    dplyr::select(ModSim, Code, Name, NumCohorts, NumAgeClassSize, agecl1:agecl20) %>%
