...
create_sim_fishery_wtage_subannual <- function(atlmod,fitstart=NULL,fitend=NULL,saveToData=T) {
  ...
  fish_wtage <- atlantisom::read_savedfisheries(file.path(d.name,v.name), 'catchWtage')
  fish_annage_wtage <- atlantisom::read_savedfisheries(file.path(d.name,v.name), 'catchAnnWtage')
  
  #join to get a full set of annual wt at age
  # by fishery, make same list structure, add this to atlantisom?
  if (length(fish_annage_wtage) > 0){
  ...
  if (saveToData) {
    ...
    usethis::use_data(simFisheryWtatAgeSubannual, overwrite = TRUE)
  }
  
  return(simFisheryWtatAgeSubannual)
