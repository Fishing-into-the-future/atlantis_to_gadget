...
## Initial numbers at age 1, using end of 1959
## TODO: add to mfdb
...
init.num.age1 <- 
  ...
  filter(time == 60) %>% 
  group_by(agecl) %>% 
  summarise(n = sum(atoutput), .groups = 'drop') %>% 
  mutate(n = n/10000) %>% 
  filter(agecl == 1) %>% 
  pull(n)
...
#   #area          = areas,
#   timestep      = defaults_base$timestep,
#   year          = defaults_base$year,
#   sampling_type = c('IGFS'),
#   data_source   = 'iceland-aldist-IGFS',
#   species       = defaults_base$species,
...
#   aldist.igfs[[1]] %>% 
#   mutate(len2 = as.numeric(substring(length,4,6)),
#          age2 = as.numeric(substring(age,4,5))) %>%
...
#   #area            = defaults_base$area,
#   timestep        = defaults_base$timestep,
#   year            = year_range,
#   species         = defaults_base$species,
#   age             = mfdb_interval('age', seq(1, 12, 1)),
#   #sampling_type   = 'INIT',
...
#   #area            = defaults_base$area,
#   #timestep        = defaults_base$timestep,
#   year            = 1960, # avgRec stored in year1
#   #species         = defaults_base$species,
#   age             = mfdb_interval('age', seq(0, 12, 1)),
#   #sampling_type   = 0,
